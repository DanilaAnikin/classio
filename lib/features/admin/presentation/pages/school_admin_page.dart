...
/// School Administration Page for BigAdmin/Principal role.
///
/// Features:
/// - Tab 1 "Users": List of profiles in current user's school
///   - Each user card shows: avatar/initials, name, email, role badge
///   - Users grouped by role (teachers, students, parents)
///   - FAB to generate invite codes
/// - Tab 2 "Classes": List of classes in the school
///   - Each class card shows: class name, grade level, academic year
///   - FAB to add new classes
...
class _SchoolAdminPageState extends ConsumerState<SchoolAdminPage>
  ...
  Widget? _buildFAB(BuildContext context, String schoolId, bool isPlayful) {
    ...
    if (_tabController.index == 0) {
      ...
      return FloatingActionButton.extended(
        onPressed: () => _showInviteUserDialog(context, schoolId),
      ...
      return FloatingActionButton.extended(
        onPressed: () => _showAddClassDialog(context, schoolId),
  ...
  void _showInviteUserDialog(BuildContext context, String schoolId) {
    showDialog(
      context: context,
      builder: (context) => _InviteUserDialog(schoolId: schoolId),
    );
  }

  void _showAddClassDialog(BuildContext context, String schoolId) {
    showDialog(
      context: context,
      builder: (context) => _AddClassDialog(schoolId: schoolId),
    );
  }
}

/// Users Tab - Displays list of users in the school grouped by role
      ...
      child: classesAsync.when(
        data: (classes) {
          ...
          for (final classEntity in classes) {
            groupedClasses.putIfAbsent(classEntity.gradeLevel, () => []);
            groupedClasses[classEntity.gradeLevel]!.add(classEntity);
          }

          // Sort grade levels
          final sortedGrades = groupedClasses.keys.toList()..sort();
            ...
            child: Column(
              ...
              children: [
                ...
                ...sortedGrades.expand((grade) => [
                      ...
                      SizedBox(height: isPlayful ? 12 : 8),
                      ...groupedClasses[grade]!.map((classEntity) => Padding(
                            padding: EdgeInsets.only(bottom: isPlayful ? 10 : 8),
                            child: _ClassCard(
...
/// Dialog for inviting users (generating invite codes)
class _InviteUserDialog extends ConsumerStatefulWidget {
  const _InviteUserDialog({required this.schoolId});
...
class _InviteUserDialogState extends ConsumerState<_InviteUserDialog> {
  ...
  Future<void> _generateCode() async {
    ...
    if (usageLimit == null || usageLimit < 1) {
      setState(() {
        _errorMessage = 'Please enter a valid usage limit (1 or more)';
      });
      return;
    }
...
class _AddClassDialog extends ConsumerStatefulWidget {
  const _AddClassDialog({required this.schoolId});
...
class _AddClassDialogState extends ConsumerState<_AddClassDialog> {
  ...
  Future<void> _createClass() async {
    ...
    final gradeLevel = int.tryParse(_gradeLevelController.text);
    if (gradeLevel == null) {
      setState(() {
        _errorMessage = 'Grade level must be a number';
      });
      return;
    }
              ...
              TextFormField(
                ...
                validator: (value) {
                  ...
                  if (int.tryParse(value) == null) {
                    return 'Grade level must be a number';
                  }
                  return null;
                },
              ),
